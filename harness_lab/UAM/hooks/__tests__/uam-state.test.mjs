...
import { deepMerge, readState, writeState, isUamActive, initState, checkConvergence } from '../lib/uam-state.mjs';
...
describe('readState / writeState', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'uam-test-'));
  });
  ...
  it('should create .uam directory if missing', () => {
    writeState(tmpDir, { current_phase: 'phase1-plan' });
    assert.ok(existsSync(join(tmpDir, '.uam')));
  });

  it('should return null for corrupt JSON (M5 schema validation)', () => {
    mkdirSync(join(tmpDir, '.uam'), { recursive: true });
    writeFileSync(join(tmpDir, '.uam', 'state.json'), 'not json{{{');
    const result = readState(tmpDir);
    assert.strictEqual(result, null);
  });
  ...
  it('should return null for valid JSON without current_phase (M5)', () => {
    mkdirSync(join(tmpDir, '.uam'), { recursive: true });
    writeFileSync(join(tmpDir, '.uam', 'state.json'), JSON.stringify({ foo: 'bar' }));
    const result = readState(tmpDir);
    assert.strictEqual(result, null);
  });
  ...
  it('should return null for JSON array (M5)', () => {
    mkdirSync(join(tmpDir, '.uam'), { recursive: true });
    writeFileSync(join(tmpDir, '.uam', 'state.json'), JSON.stringify([1, 2, 3]));
    const result = readState(tmpDir);
    assert.strictEqual(result, null);
  });
  ...
  it('should use atomic write (temp file + rename)', () => {
    ...
    writeState(tmpDir, { current_phase: 'phase1-plan' });
    const stateDir = join(tmpDir, '.uam');
...
describe('isUamActive', () => {
  ...
  it('should return false when no state file exists', () => {
    assert.strictEqual(isUamActive(tmpDir), false);
  });

  it('should return true for active phase', () => {
    writeState(tmpDir, { current_phase: 'phase2-sprint' });
    assert.strictEqual(isUamActive(tmpDir), true);
  });
  ...
  it('should return true for corrupt state file (M6 fail-closed)', () => {
    mkdirSync(join(tmpDir, '.uam'), { recursive: true });
    writeFileSync(join(tmpDir, '.uam', 'state.json'), 'corrupt data!!!');
    assert.strictEqual(isUamActive(tmpDir), true);
  });
});
...
describe('initState', () => {
  ...
  it('should create state file with correct defaults (full mode)', () => {
    ...
    assert.ok(state.pipeline_id.startsWith('uam-'));
  ...
  it('should persist state file to disk', () => {
    ...
    assert.ok(existsSync(join(tmpDir, '.uam', 'state.json')));
    const raw = JSON.parse(readFileSync(join(tmpDir, '.uam', 'state.json'), 'utf-8'));
