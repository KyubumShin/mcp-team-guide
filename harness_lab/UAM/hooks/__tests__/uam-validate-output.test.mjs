...
describe('uam-validate-output constants', () => {
...
describe('validation logic contract', () => {
  it('should define correct agent types requiring validation', () => {
    ...
    assert.ok(expectedAgents.includes('uam-worker'));
    assert.ok(expectedAgents.includes('uam-gap-analyzer'));
  });
  ...
  it('C3: validation failure should produce [VALIDATION FAILED] prefix', () => {
    ...
    const failedMessage = '[VALIDATION FAILED] [UAM VALIDATION FAILED] Agent "uam-worker" output is missing sections.';
