...
import { isAllowedPath, isSourceFile } from '../uam-write-guard.mjs';

describe('isAllowedPath', () => {
  it('should allow .uam/ paths', () => {
    assert.strictEqual(isAllowedPath('.uam/state.json'), true);
    assert.strictEqual(isAllowedPath('/project/.uam/config.json'), true);
  });
  ...
  it('should allow UAM/ plugin directory', () => {
    assert.strictEqual(isAllowedPath('/project/UAM/hooks/test.mjs'), true);
  });
