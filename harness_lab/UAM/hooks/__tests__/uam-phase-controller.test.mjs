...
import { checkPlanStatus, checkGateResults } from '../uam-phase-controller.mjs';

describe('checkPlanStatus', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'uam-plan-test-'));
  });
  ...
  it('should detect all checked items', () => {
    ...
    writeFileSync(join(tmpDir, '.uam', 'PLAN.md'), `
### [x] Task 1
### [X] Task 2
### [x] Task 3
`);
...
`);
  ...
  it('should return total=0 for empty PLAN.md', () => {
    mkdirSync(join(tmpDir, '.uam'), { recursive: true });
    writeFileSync(join(tmpDir, '.uam', 'PLAN.md'), '# My Plan\nSome notes');
    const result = checkPlanStatus(tmpDir);
