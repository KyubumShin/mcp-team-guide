  ...
  switch (phase) {
    ...
    case 'phase1a-research': {
      ...
      if (research.status === 'completed' || research.status === 'skipped') {
        ...
        const msg = research.status === 'skipped'
          ? '[UAM] Research skipped. Transitioning to Phase 1-B: Plan Generation.'
          : `[UAM] Research completed (${research.stages_completed?.length || 0} stages, ${research.findings_count || 0} findings, ${research.sources_count || 0} sources). Transitioning to Phase 1-B: Plan Generation.`;
        console.log(JSON.stringify({
          continue: true,
          hookSpecificOutput: {
            hookEventName: 'Stop',
        ...
        console.log(JSON.stringify({
          ...
          hookSpecificOutput: {
            hookEventName: 'Stop',
            additionalContext: `[UAM] Phase 1-A: Deep Research in progress (stage: ${currentStage}, ${stagesCompleted}/3 stages completed). Complete all research stages or skip to proceed.`
          }
        }));
      }
      break;
    ...
    case 'phase1b-plan': {
      ...
      console.log(JSON.stringify({
        ...
        hookSpecificOutput: {
          hookEventName: 'Stop',
          additionalContext: `[UAM] Phase 1-B: Plan Generation in progress.${reportNote} Use research findings as input for planning agents. Complete PLAN.md and HITL before proceeding.`
        }
      }));
      break;
    }
